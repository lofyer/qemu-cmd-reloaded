...
KVM_CMD="qemu-kvm -no-user-config -nodefaults"
...
FLAG_INNER_NET="-net tap,ifname=tap0,script=no,downscript=no -net nic,model=virtio,macaddr=94:de:80:7a:30:85"
FLAG_CPU_TYPE="-cpu host"
FLAG_CPU_FEATURE="-smp 4,sockets=1,cores=4"
FLAG_MM="-m 2048M"
#FLAG_CD="-cdrom centos.iso"
FLAG_KB="-k en-us"
...
#	DISPLAY OPTIONS		#
#################################
FLAG_SPICE="-spice port=6000,ipv4,disable-ticketing"
FLAG_VGA="-vga qxl"
...
FLAG_VIRTIO_CDROM="-drive file=centos.iso,if=none,id=drive-ide0-1-0,readonly=on,format=raw,serial= -device ide-cd,bus=ide.1,unit=0,drive=drive-ide0-1-0,id=ide0-1-0"
FLAG_VIRTIO_HDA="-drive file=hda.raw,if=none,id=drive-virtio-disk0,format=raw,serial=8ca429f5-285d-41b6-8ce9-ec74a519e3b9,cache=none,werror=stop,rerror=stop,aio=threads -device virtio-blk-pci,scsi=off,bus=pci.0,addr=0x7,drive=drive-virtio-disk0,id=virtio-disk0,bootindex=1"
